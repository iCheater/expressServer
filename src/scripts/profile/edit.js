...
function requestDataInputs (data) {
// eslint-disable-next-line no-undef
  const xhr = new XMLHttpRequest()
  xhr.onload = function (e) {
    if (xhr.status >= 200 && xhr.status < 300) {
...
function changeData (data) {
  ...
  const link = block.querySelector('.profile-edit')
  link.innerHTML = 'Сохранить'
  link.id = 'save'
  link.style.color = '#3ac267'
  ...
  block.addEventListener('click', () => {
    if (link[0].id === 'save') {
      console.log('click')
      blockInfoChildren.forEach((element) => {
        const nameVariable = element.id
